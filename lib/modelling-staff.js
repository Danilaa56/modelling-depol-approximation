...
function createDataView(label, parameterName, getter, suffix) {
...
<div class="dataHolder">
    <div>${label}:</div>
    <div>
        <span id="${elementId}">${getter()}</span>
        <span>${suffix}</span>
    </div>
</div>
`);

    let valueSpan = document.getElementById(elementId);
    context.data[parameterName] = {getter: getter, view: valueSpan};
}
...
`);
    ...
    context.input[parameterName] = {validator: validator, value: undefined, view: input, setter: setter};
    input.addEventListener("input", (e) => updateInput(e, context.input[parameterName]));
    setter(initialValue);
}
